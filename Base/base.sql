...
DROP VIEW IF EXISTS view_personnel_user;

-- Créer la vue
CREATE OR REPLACE VIEW view_personnel_user AS
SELECT 
    p.id_personnel,
    u.id_user,
    d.id_departement
FROM personnel p 
JOIN poste ps ON p.id_poste = ps.id_poste 
JOIN departement d ON ps.id_departement = d.id_departement 
JOIN (
    SELECT u.* 
    FROM user_ u 
    JOIN role r ON r.id_role = u.id_role 
    WHERE r.libelle = 'Chef de département'
) u ON d.id_departement = u.id_departement;
-- ============================================================================
...
-- Trigger: Avant insertion d'une validation chef
...
BEGIN
    ...
    -- =========================
    -- 1. Anti-auto-validation pour les présences
    -- =========================
    ...
    -- =========================
    -- 2. Chef valide seulement son département (pour absences)
    -- =========================
    IF NEW.Id_justification_absence IS NOT NULL THEN
        ...
        IF id_departement_personnel IS NULL OR id_departement_chef != id_departement_personnel THEN
            SIGNAL SQLSTATE '45000'
    ...
    -- =========================
    -- 3. Chef valide seulement son département (pour retards)
    -- =========================
...
-- Trigger: Après insertion d'une validation chef
DELIMITER //
...
BEGIN
    ...
    -- Si accepté par le chef, mettre à jour est_justifie temporairement
...
-- Trigger: Validation RH après insertion
DELIMITER //
...
BEGIN
    ...
    SELECT Id_justification_absence, Id_justification_retard
    INTO id_justif_absence, id_justif_retard
    ...
    IF decision_libelle = 'refusé' THEN
        ...
        END IF;
    END IF;
    -- Si RH accepte, les justifications restent TRUE (déjà marqué par le chef)
END//
DELIMITER ;
...
BEGIN
    ...
    -- **METHODE ALTERNATIVE sans utiliser la vue**
    -- Récupérer le département du personnel
    SELECT d.id_departement INTO dept_id
    FROM personnel p
    ...
    -- Si département trouvé, chercher le chef de département
    IF dept_id IS NOT NULL THEN
        SELECT u.id_user INTO user_id
        FROM user_ u
        JOIN role r ON r.id_role = u.id_role
        WHERE u.id_departement = dept_id
          AND r.libelle = 'Chef de département'
        LIMIT 1;
    END IF;
    ...
    THEN
        ...
        IF salaire > 0 THEN
            ...
            WHERE phs.id_personnel = personnel_id
              AND DATE(hs.date_) = DATE(NEW.date_);
...
-- 1. ÉVÉNEMENT QUOTIDIEN POUR GÉRER LES ABSENCES AUTOMATIQUES
DELIMITER //

CREATE EVENT IF NOT EXISTS event_gestion_absences_automatique
ON SCHEDULE EVERY 1 DAY
STARTS CONCAT(DATE_ADD(CURDATE(), INTERVAL 1 DAY), ' 08:00:00') -- S'exécute chaque jour à 8h
COMMENT 'Gestion automatique des absences quotidiennes'
DO
BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE personnel_id INT;
    DECLARE date_aujourdhui DATE DEFAULT CURDATE();
    DECLARE cur_personnel CURSOR FOR 
        SELECT Id_personnel FROM personnel WHERE actif = TRUE;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    
    -- Log de début
    INSERT INTO audit_logs (table_name, record_id, action, details)
    VALUES ('event', 0, 'start', CONCAT('Début gestion absences pour ', date_aujourdhui));
    
    OPEN cur_personnel;
    
    read_loop: LOOP
        FETCH cur_personnel INTO personnel_id;
        IF done THEN
            LEAVE read_loop;
        END IF;
        
        -- Vérifier si une présence existe pour ce personnel aujourd'hui
        IF NOT EXISTS (
            SELECT 1 
            FROM presence_absence 
            WHERE Id_personnel = personnel_id 
            AND date_ = date_aujourdhui
        ) THEN
            -- Étape 1: Créer une entrée dans presence_absence avec NULL
            INSERT INTO presence_absence (
                date_, 
                heure_arrivee, 
                heure_depart, 
                present, 
            ...
            -- Étape 2: Créer une justification d'absence automatique
            INSERT INTO justification_absence (
                date_demande,
                date_absence,
                est_justifie,
                ...
                date_aujourdhui,
                date_aujourdhui,
                FALSE, -- Non justifié par défaut
                personnel_id,
                'Absence automatique - système'
            );
            
            -- Log de l'action
            INSERT INTO audit_logs (table_name, record_id, action, details)
            VALUES ('personnel', personnel_id, 'auto_absence', 
                   CONCAT('Absence automatique créée pour ', date_aujourdhui));
        END IF;
    END LOOP;
    ...
    -- Log de fin
    INSERT INTO audit_logs (table_name, record_id, action, details)
    VALUES ('event', 0, 'end', CONCAT('Fin gestion absences pour ', date_aujourdhui));
